...
const CustomerDetails: React.FC = () => {
  ...
  const handleUploadPreferred = (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const deletePreferred = (prefId: string) => {
    if (!customer) return;
        ...
        <div className="max-w-6xl mx-auto">
          <header className="mb-12 flex flex-col md:flex-row md:items-center justify-between gap-8 bg-white p-10 rounded-[50px] border border-[#f6c1cc] shadow-sm">
            ...
            <div className="flex items-center gap-5">
               {saveStatus && (
                 <div className="bg-green-500 text-white px-6 py-3 rounded-full text-[10px] font-black uppercase tracking-[0.3em] shadow-xl animate-in zoom-in">
                   Synchronized
                 </div>
               )}
               <button 
