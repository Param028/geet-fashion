...
const AdminCustomers: React.FC = () => {
  ...
  const loadCustomers = async () => {
    try {
      ...
      showToast("Failed to retrieve client registry.", "error");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAddCustomer = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setIsAdding(false);
      showToast('Client record created successfully.', 'success');
    } catch (error: any) {
      showToast(error.message || "Failed to save client record.", "error");
    }
  };
  ...
  const confirmDelete = async () => {
    if (!deleteId) return;
    try {
      await storage.deleteCustomer(deleteId);
      await loadCustomers();
      showToast('Client record permanently purged.', 'success');
    } catch (error: any) {
      showToast("Failed to delete record. Cloud sync error.", "error");
    } finally {
      setDeleteId(null);
    }
  };
  ...
  return (
    <AdminLayout>
      ...
      <ConfirmationModal 
        ...
        message="This action will permanently delete the client's profile, measurements history, and preferred designs. This cannot be undone."
        ...
        <div className="overflow-x-auto">
          {loading ? (
             <div className="py-20 text-center">
                <div className="animate-spin text-4xl text-[#c9a14a] mb-4">⟳</div>
                <p className="text-gray-400 font-black uppercase text-xs tracking-widest">Syncing Registry...</p>
             </div>
          ) : (
