...
const AdminSidebar: React.FC = () => {
  ...
  const SidebarContent = () => (
    <div className="p-8 h-full flex flex-col">
      <div className="mb-12 flex items-center gap-4 pl-2">
        <div className="w-12 h-12 bg-white rounded-full flex items-center justify-center border border-[#f6c1cc] overflow-hidden shadow-lg p-0.5 shrink-0">
           {!imgError ? (
             <img 
               src={LOGO_SRC} 
               alt="Geet Fashion" 
          ...
          return (
            <Link
              ...
              className={`flex items-center space-x-4 p-4 rounded-2xl transition-all duration-300 group ${
                isActive 
                  ? 'bg-[#4a2c2a] text-white shadow-xl translate-x-2' 
      ...
      <div className="mt-auto pt-10 border-t border-[#fff7f9]">
        <button 
          ...
          className="w-full flex items-center space-x-4 p-4 rounded-2xl text-red-300 hover:bg-red-50 hover:text-red-500 transition-all font-black text-[10px] uppercase tracking-[0.3em] group"
        >
  ...
  return (
    <>
      {/* Mobile Header */}
      <div className="md:hidden fixed top-0 left-0 right-0 h-20 bg-white/90 backdrop-blur-md border-b border-[#f6c1cc] flex items-center justify-between px-6 z-[60] shadow-sm">
      ...
      {/* Mobile Drawer */}
      <aside className={`md:hidden fixed top-0 left-0 h-full w-72 bg-white z-[60] shadow-2xl transition-transform duration-500 ease-[cubic-bezier(0.16,1,0.3,1)] ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <SidebarContent />
      </aside>

      {/* Desktop Sidebar */}
      <aside className="w-72 bg-white min-h-screen border-r border-[#f6c1cc] hidden md:block fixed left-0 top-0 z-50">
        <SidebarContent />
      </aside>
    </>
  );
